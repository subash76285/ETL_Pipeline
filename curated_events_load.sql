...
create or replace table curatedds.curated_events_final as select user_pseudo_id, event_name, event_time, source, medium, event_value, ROW_NUMBER() OVER (PARTITION by user_pseudo_id order by event_time) AS event_order from curatedds.curated_events;
